...
TARGETS = {
    "8e8y": {
        "candidates": ["8e8y.pdb", "8E8Y.pdb"],
        "apply_mutations": False,
        "keep_chains": ["A"],
        "interface_partner_chains": [],
        "keep_hetero": ["PLM"],
        "extra_res_fa": "PLM.params",
        "final_output": "8E8Y_VP1_PLM_final.pdb",
        "chain_renamed_output": "8E8Y_VP1_PLM_final_ligX.pdb",
        "ligand_resname": "PLM",
        "ligand_chain": "X",
    },
}
...
def rename_ligand_chain(input_pdb, output_pdb, ligand_resname, new_chain):
    with open(input_pdb, "r", encoding="utf-8") as infile, open(output_pdb, "w", encoding="utf-8", newline="\n") as outfile:
        for line in infile:
            if line.startswith("HETATM") and ligand_resname in line:
                line = line[:21] + new_chain + line[22:]
            outfile.write(line)
    print(f"Renamed ligand {ligand_resname} to chain {new_chain}: {output_pdb}")
...
def cleanup_target_outputs(tag, scorefile, final_output, extra_outputs=None):
    for old_model in glob.glob(f"{tag}_clean_relaxed_*.pdb"):
        try:
            os.remove(old_model)
        except OSError:
            pass
    for path in [scorefile, final_output] + list(extra_outputs or []):
...
def process_target(tag, config, source_pdb, rosetta_scripts, seq_mutations, xml_path, cpu_threads):
    ...
    chain_renamed_output = config.get("chain_renamed_output")

    print(f"\n=== {tag} ({source_pdb}) ===")
    cleanup_target_outputs(tag, scorefile, final_output, extra_outputs=[chain_renamed_output] if chain_renamed_output else [])

    keep_hetero = {"ZN", "MG", "CA", "NA", "K", "CL"}
    keep_hetero.update(config.get("keep_hetero", []))

    clean_and_prepare_pdb(
        source_pdb,
        clean_pdb,
        keep_hetero=sorted(keep_hetero),
    ...
    ligand_params = config.get("extra_res_fa")
    if ligand_params:
        if not os.path.exists(ligand_params):
            return tag, False, f"Missing ligand params file: {ligand_params}"
        cmd.extend(["-extra_res_fa", ligand_params])
    ...
    if chain_renamed_output:
        rename_ligand_chain(
            final_output,
            chain_renamed_output,
            config.get("ligand_resname", "PLM"),
            config.get("ligand_chain", "X"),
        )
        return tag, True, chain_renamed_output
    return tag, True, final_output


def main():
    requires_mutations = any(config.get("apply_mutations") for config in TARGETS.values())
    if requires_mutations:
    ...
    seq_mutations = parse_mutations(MUTATION_FILE) if requires_mutations else []
    xml_path = "relax_mutations.xml"
    write_relax_xml(xml_path)

    print("Preparing VP1 chain with ligand retention and running constrained Cartesian relax...")
    ...
    print("\nDone. VP1-ligand complex prepared.")
